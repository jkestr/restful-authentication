...
<% elsif options[:stateful] -%>
  ...
  validates_presence_of     :login, :if => :member?
  validates_length_of       :login, :within => 3..40, :if => :member?
  validates_uniqueness_of   :login, :scope => :member, :if => :member?
  validates_format_of       :login, :if => :member?, 
                            :with => Authentication.login_regex, 
                            :message => Authentication.bad_login_message

  validates_presence_of     :name_first
  validates_format_of       :name_first, :with => Authentication.name_regex,  
                            :message => Authentication.bad_name_message

  validages_presence_of     :name_last
  validates_format_of       :name_last, :with => Authentication.name_regex,
                            :message => Authentication.bad_name_message

  validates_presence_of     :email
  validates_length_of       :email, :within => 6..100 #r@a.wk
  validates_uniqueness_of   :email, :scope => :member, :if => :member?
  ...
  attr_accessible :login, :email, :name_first, :name_last, 
